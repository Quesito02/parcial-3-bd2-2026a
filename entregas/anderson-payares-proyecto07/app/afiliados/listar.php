...
try {
    ...
    try {
        $stmt = $pdo->query("SELECT id, documento, nombre, apellido, somatotipo, objetivo FROM afiliados ORDER BY id DESC");
        $atletas = $stmt->fetchAll();
...
<head>
    ...
    <style>
        ...
        .search-box { background-color: #151719 !important; border: 1px solid #3c4248 !important; color: #fff !important; border-radius: 30px !important; padding: 12px 20px 12px 45px !important; }
        .search-box:focus { border-color: #ffc107 !important; box-shadow: 0 0 0 0.25rem rgba(255, 193, 7, 0.2) !important; }
...
<body>
    ...
    <div class="container py-5">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
                <a href="nuevo.php" class="btn btn-warning rounded-pill btn-sm px-3 fw-bold text-dark me-1"><i class="bi bi-person-plus-fill me-1"></i> Agregar Atleta</a>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach($atletas as $atleta): 
                                $idReal = isset($atleta['id_afiliado']) ? $atleta['id_afiliado'] : $atleta['id'];
                                $estadoId = $idReal % 3;
                                if ($estadoId === 0) {
                                ...
                                <tr class="fila-atleta">
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm">
                                            <a href="ver.php?id=<?php echo $idReal; ?>" class="btn btn-info fw-bold text-dark" title="Ver Ficha Técnica">
                                                <i class="bi bi-eye-fill"></i>
                                            </a>
                                            <a href="../asistencias/registrar.php?id_afiliado=<?php echo $idReal; ?>" class="btn btn-warning fw-bold text-dark" title="Registrar Entrada">
                                                <i class="bi bi-door-open-fill"></i>
                                            </a>
                                            <a href="editar.php?id=<?php echo $idReal; ?>" class="btn btn-outline-light" title="Editar Atleta">
                        ...
                        <tr id="filaNoResultados" style="display: none;">
                            <td colspan="6" class="text-center text-muted py-4">No se encontraron atletas coincidentes.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <script>
        document.getElementById('inputBuscador').addEventListener('input', function() {
            ...
            document.getElementById('filaNoResultados').style.display = (encontrados === 0 && busqueda !== '') ? '' : 'none';
        });
    </script>
</body>
</html>